...
export default function ActivityPanel({ actions, catalog, selected, onToggle, onUpdate }: { actions?: string[]; catalog?: string[]; selected: string[]; onToggle: (label: string) => void; onUpdate: (actions: string[], catalog: string[]) => void }) {
  ...
  return <section className="activity-panel glass-card" aria-labelledby="activity-title">
    <header className="activity-heading"><div><p className="eyebrow">действия дня</p><h2 id="activity-title">Моя активность</h2><p>Ритм, движение и контекст дня — отдельно от отметок цикла.</p></div><div className="activity-heading-actions"><button className="activity-info-button" type="button" aria-label="Как работает блок «Моя активность»" aria-expanded={infoOpen} onClick={() => setInfoOpen((value) => !value)}>i</button><button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}>{open ? "готово" : "＋ настроить"}</button></div></header>
    ...
    <p className="activity-question">Что было сегодня?</p>

...
const SELECTOR_PHASE: Record<LotusStage["key"], string> = { menstruation: "Месячные", low: "Низкая фаза", follicular: "Фолликул.", ovulation: "Овуляция" };
...
function arcPoint(offset: number) {
  ...
  return { x: 190 + Math.sin(angle) * 187, y: 27 + (1 - Math.cos(angle)) * 104 };
}
...
export default function CycleHero({ profile, days, activeIndex, onSelectDay, onOpenPeriod }: {
  profile: AlmaProfile; days: DayModel[]; activeIndex: number; onSelectDay: (index: number) => void; onOpenPeriod: () => void;
}) {
  ...
  useEffect(() => {
    if (!isDragging) { previewIndexRef.current = activeIndex; setPreviewIndexState(activeIndex); }
  ...
  function scheduleDateClick(delay = 0) {
    ...
    oscillator.type = "triangle"; oscillator.frequency.setValueAtTime(920, start); oscillator.frequency.exponentialRampToValueAtTime(460, start + .026);
    gain.gain.setValueAtTime(.0001, start); gain.gain.exponentialRampToValueAtTime(.032, start + .002); gain.gain.exponentialRampToValueAtTime(.0001, start + .034);
    oscillator.connect(gain); gain.connect(context.destination); oscillator.start(start); oscillator.stop(start + .036);
  }
  ...
  function onPointerDown(event: PointerEvent<SVGSVGElement>) {
    event.preventDefault(); getAudioContext(); dragStart.current = event.clientX; dragged.current = false; previewIndexRef.current = activeIndex;
    setPreviewIndexState(activeIndex); setIsDragging(true); event.currentTarget.setPointerCapture(event.pointerId);
  }
  ...
  function onPointerMove(event: PointerEvent<SVGSVGElement>) {
    ...
    if (dragFrame.current == null) {
      dragFrame.current = requestAnimationFrame(() => {
        ...
        // Geometry is painted first and never waits for React. This keeps the arc locked 1:1 to the finger.
        paintDial(nextDragX);
        const nextIndex = clampIndex(activeIndex + Math.round(-nextDragX / DOT_STEP), days);
        if (nextIndex !== previewIndexRef.current) {
          previewIndexRef.current = nextIndex;
          // Flower/date/phase are visual preview work; keep them non-blocking while the pointer is moving.
          startTransition(() => setPreviewIndexState(nextIndex));
        }
  ...
  function finishDrag() {
    if (dragStart.current == null) return;
    const shift = Math.round(-dragXRef.current / DOT_STEP); const nextIndex = clampIndex(activeIndex + shift, days);
    if (nextIndex !== previewIndexRef.current) clickCrossedDates(previewIndexRef.current, nextIndex);
    previewIndexRef.current = nextIndex;
    if (dragFrame.current != null) cancelAnimationFrame(dragFrame.current);
    dragFrame.current = null; dragStart.current = null; dragXRef.current = 0; setPreviewIndexState(nextIndex); setIsDragging(false);
    if (nextIndex !== activeIndex) select(nextIndex);
  }

  function cancelDrag() {
    if (dragFrame.current != null) cancelAnimationFrame(dragFrame.current);
    dragFrame.current = null; dragStart.current = null; dragXRef.current = 0; dragged.current = false; previewIndexRef.current = activeIndex;
    paintDial(0); setPreviewIndexState(activeIndex); setIsDragging(false);
  }

  function onDialKeyDown(event: KeyboardEvent<SVGSVGElement>) {
    if (event.key === "ArrowLeft" || event.key === "ArrowRight") { event.preventDefault(); const nextIndex = clampIndex(activeIndex + (event.key === "ArrowRight" ? 1 : -1), days); if (nextIndex !== activeIndex) scheduleDateClick(); select(nextIndex); }
  }

  function selectDot(index: number) { if (dragged.current) { dragged.current = false; return; } if (index !== activeIndex) scheduleDateClick(); select(index); }
  ...
  return <section className={`cycle-hero lotus-stage-${previewStage.key}${isDragging ? " is-cycle-scrubbing" : ""}`} style={customStyle} aria-labelledby="cycle-title" data-lotus-stage={previewStage.key} data-lotus-petals={previewStage.petals} data-selected-index={activeIndex} data-preview-index={previewIndex}>
    ...
    <svg className={`cycle-dial${isDragging ? " is-dragging" : ""}`} viewBox="0 0 380 142" role="group" tabIndex={0}
      ...
      onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishDrag} onPointerCancel={cancelDrag} onKeyDown={onDialKeyDown}>
      ...
      {visibleDays.map(({ item, index, offset }) => {
        const visualOffset = offset + (isDragging ? dragXRef.current / DOT_STEP : 0); const point = arcPoint(visualOffset); const dotStage = stageForPhase(item.phase);
        const isNear = Math.abs(visualOffset) <= 6.45; const month = item.dayOfMonth === 1 ? MONTH_SHORT.format(item.date).replace(".", "") : "";
        return <g key={item.iso} ref={(element) => { if (element) dialDateRefs.current.set(index, element); else dialDateRefs.current.delete(index); }}
          className={`dial-date${index === previewIndex ? " is-selected" : ""}${item.isToday ? " is-today" : ""}`} transform={`translate(${point.x} ${point.y})`}
          style={{ "--dot-color": dotStage.color, opacity: isNear ? Math.max(.18, 1 - Math.abs(visualOffset) / 11) : 0, visibility: isNear ? "visible" : "hidden", pointerEvents: isNear ? "auto" : "none" } as CSSProperties}
          role="button" tabIndex={isNear ? 0 : -1} aria-current={index === previewIndex ? "date" : undefined} aria-label={`${item.dayOfMonth} ${MONTH_SHORT.format(item.date)}, ${item.cycleDay} день цикла`}
          onClick={() => selectDot(index)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectDot(index); } }}>
      ...
      <g className="dial-selector" transform="translate(190 29)" style={{ "--selector-color": previewStage.color } as CSSProperties} aria-hidden="true">
        <circle className="dial-selector-aura" r="26" filter="url(#dial-glow)" /><circle className="dial-selector-core" r="22" />
        ...
        <text className="dial-selector-phase" y="2" textAnchor="middle">{SELECTOR_PHASE[previewStage.key]}</text><text className="dial-selector-cycle" y="12" textAnchor="middle">день цикла {previewDay.cycleDay}</text>
      </g>
    </svg>
    ...
    <button className="cycle-lotus-button" type="button" onClick={onOpenPeriod} aria-label="Открыть календарь и отметить месячные">
      <svg className="cycle-lotus" viewBox="0 0 380 230" role="img" aria-labelledby="cycle-title cycle-description">
        <defs>
          <radialGradient id="lotus-fill"><stop offset="0" stopColor={previewStage.color} stopOpacity=".38" /><stop offset=".62" stopColor={previewStage.color} stopOpacity=".12" /><stop offset="1" stopColor="#030107" stopOpacity="0" /></radialGradient>
          <linearGradient id="petal-fill" x1="0" y1="0" x2="0" y2="1"><stop stopColor={previewStage.color} /><stop offset=".42" stopColor={previewStage.color} /><stop offset=".82" stopColor="#120718" /><stop offset="1" stopColor="#050108" /></linearGradient>
          <radialGradient id="drop-fill" cx="50%" cy="35%" r="72%"><stop stopColor={previewStage.color} /><stop offset=".48" stopColor={previewStage.color} /><stop offset=".82" stopColor="#100817" /><stop offset="1" stopColor="#050108" /></radialGradient>
          <filter id="lotus-glow" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="5" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
          <filter id="pollen-glow" x="-300%" y="-300%" width="700%" height="700%"><feGaussianBlur stdDeviation="2.8" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        </defs>
        <g className="lotus-pollen-field" aria-hidden="true" filter="url(#pollen-glow)">
          {Array.from({ length: 34 }, (_, index) => { const x = 42 + ((index * 79) % 296); const y = 34 + ((index * 53) % 172); const size = .65 + (index % 5) * .34;
            return <circle key={index} className={`lotus-pollen pollen-${index % 3}`} cx={x} cy={y} r={size} style={{ "--pollen-delay": `${-(index % 11) * .73}s`, "--pollen-duration": `${5.8 + (index % 7) * .72}s`, "--pollen-x": `${((index * 17) % 25) - 12}px`, "--pollen-y": `${-12 - (index % 6) * 4}px` } as CSSProperties} />; })}
        </g>
        ...
        <g className={`lotus-bloom bloom-${previewStage.petals}`} filter="url(#lotus-glow)" aria-hidden="true">
          {lotusPetals.map((petal) => <path key={petal.id} className={`lotus-petal ${petal.className}`} style={{ "--petal-angle": `${petal.angle}deg` } as CSSProperties} d={LOTUS_PETAL_PATH} />)}
        ...
        <g className="lotus-pollen-field is-foreground" aria-hidden="true" filter="url(#pollen-glow)">
          {Array.from({ length: 18 }, (_, index) => { const x = 126 + ((index * 67) % 132); const y = 55 + ((index * 47) % 132);
            return <circle key={index} className={`lotus-pollen pollen-${(index + 1) % 3}`} cx={x} cy={y} r={.55 + (index % 4) * .31} style={{ "--pollen-delay": `${-(index % 9) * .81}s`, "--pollen-duration": `${6.4 + (index % 6) * .68}s`, "--pollen-x": `${((index * 13) % 19) - 9}px`, "--pollen-y": `${-10 - (index % 5) * 4}px` } as CSSProperties} />; })}
        </g>
